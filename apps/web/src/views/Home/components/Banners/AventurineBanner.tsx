...
const facuetLink2 = 'https://faucet.zq2-protomainnet.zilliqa.com'

export const AventurineBanner = () => {
  ...
  const faucetActions = (
    <>
      <StyledButtonLinkAction color="white" href={facuetLink} padding={['8px 12px']}>
        {t('Get proto-testnet ZIL')}
      </StyledButtonLinkAction>
      <StyledButtonLinkAction color="white" href={facuetLink2} padding={['8px 12px']} style={{ marginLeft: '8px' }}>
        {t('Get proto-mainnet ZIL')}
      </StyledButtonLinkAction>
    </>
  )
        ...
        title={
          <BannerTitle variant="purple">
            {isMobile || isTablet
              ...
              : t('Zilliqa 2.0 Aventurine proto-mainnet/proto-testnet is here. Lightning fast transactions, ahoy!')}
          </BannerTitle>
        }
        actions={<BannerActionContainer>{faucetActions}</BannerActionContainer>}
      />
      <BannerGraphics>
        <FloatingGraphic
          src={floatingAsset}
